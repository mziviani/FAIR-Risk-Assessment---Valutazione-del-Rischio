...
$$\mathbf{V\%} = \mathbf{	ext{TC}} 	imes (1 - \mathbf{	ext{CS}})$$

Se tutti i controlli sono attivi al 100% ($CS = 1$), la vulnerabilità scende a $0\%$; se non vi è alcun controllo ($CS = 0$), la vulnerabilità coincide con la capacità dell'attaccante ($TC\%$).
...
2. **Affidabilità Operativa Annua:** Rappresenta la probabilità stocastica di operare per 365 giorni senza subire alcun incidente con successo, modellata tramite la distribuzione di Poisson per eventi rari:
   $$\mathbf{	ext{Affidabilità Operativa Annua}} = \mathbf{	ext{MAX}(0; 	ext{EXP}(-	ext{LEF}))}$$
3. **Tempo di Ritorno degli Incidenti:** Intervallo medio atteso in anni tra un breach e il successivo ($T = 1 / 	ext{LEF}$).
...
9. **Regolamento UE 2017/745 (MDR):** Requisiti di sicurezza e classificazione dei dispositivi medici operativi.

---

## 6. Struttura dei File Excel della Suite

La suite comprende i seguenti fogli di calcolo operativi:

* **`Matrice_Mitigazioni_e_Vettori_FAIR-v6.xlsx`:** Matrice principale integrata contenente i 10 vettori di minaccia, la scomposizione $\Delta	ext{CF}$/$\Delta	ext{PoA}$, la configurazione dei pesi $w_i$, il calcolo $CS$ pesato e il motore stocastico `EXP(-LEF)`.
* **`Calcolatore_ALE_FAIR_EMEA.xlsx`:** Simulatore finanziario dinamico per la stima dell'ALE (minima, media, massima) in Euro con confronto Rischio Inerente vs. Residuo.
* **`Matrice_Costi_Geografici_Breach_2026_EUR.xlsx`:** Tabella pivot completa dei costi di breach espressi in cifre intere (`€ #,##0`) per le 5 macro-aree mondiali.
* **`Matrice_Tier_Criticita_Asset_FAIR-v4.xlsx`:** Classificazione a scaglioni di frequenza dagli asset Tier 3 agli asset Tier 0 (Life Critical).

---

## 7. Licenza Aperta e Feedback

Questo progetto è distribuito con **licenza aperta e libera per l'utilizzo**, la modifica e l'adattamento in contesti accademici, aziendali e professionali.

💡 **Feedback & Contributi:**  
I suggerimenti, le segnalazioni di miglioramento e le proposte di integrazione da parte di CISO, Risk Manager, Security Auditor ed appassionati di modellazione quantitativa sono estremamente graditi ed incoraggiati!

---

*Generated by Gemini Notebook | Model Version v6.0 (FAIR & ISO/IEC 27005:2022 Compliant)*
